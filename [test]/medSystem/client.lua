...
local cNameL = ""
local cArea = ""
local cBleeding = "Нет кровотечения"
...
AddEventHandler('esx:onPlayerDeath', function(data)
	...
	if (bone == 31086) then
		multi = 0.0
		print('Kafa')
		TriggerEvent('chatMessage', "MedSystem", {255, 0, 0}, "Вы были убиты выстрелом в голову.")
		bleeding = 5
		area = "Голова"
	end
	if bone == 24817 or bone == 24818 or bone == 10706 or bone == 24816 or bone == 11816 then
		multi = 1.0
		print('Тело')
		TriggerEvent('chatMessage', "MedSystem", {255, 0, 0}, "Вы были убиты выстрелом в тело")
		bleeding = 2
		area = "Туловище"
	end
...
AddEventHandler('medSystem:near', function(x,y,z, pulse, blood, nameF, nameL, area, bldn)
	...
	if GetDistanceBetweenCoords(x,y,z,a,b,c,false) < 10 then
		timer = 10
		TriggerEvent('chatMessage', "MedSystem", {255,0 , 0}, "^1 [0-5] Мертв ^6[5-15] Требуется везти в больницу ^5[15-38] Требуется помощь врача на месте ^4[38-55] Полиция может помочь ^2[55+] Здоров")
		cBlood = blood
		cPulse = pulse
		cNameF = nameF
		cNameL = nameL
		...
		Wait(1)
			while timer >= 1 do
				...
				if cPulse ~= -1 and cBlood ~= -1 then
					DrawAdvancedText(0.7, 0.7, 0.005, 0.0028, 0.9, cNameF .. " " .. cNameL .. "\n~r~Пульс: ~w~" .. cPulse .. "BPM\n~r~Kan: ~w~" .. cBlood .. "%~r~\nРаненый участок: ~w~" .. cArea .. "\n~r~Кровотечение: ~w~" .. cBleeding, 255, 255, 255, 255, 4, 1)
