...
Citizen.CreateThread(function()
    while true do 
		...
		--IsVehicleSeatFree(Vehicle.Vehicle, -1) and
                if GetVehicleEngineHealth(Vehicle.Vehicle) <= 100 then
                    ShowNotification('Нажмите [~g~SHIFT~w~] и [~g~E~w~] чтобы толкать транспорт')
                end
     
            --and IsVehicleSeatFree(Vehicle.Vehicle, -1)
            if IsControlPressed(0, Keys["LEFTSHIFT"]) and not IsEntityAttachedToEntity(ped, Vehicle.Vehicle) and IsControlJustPressed(0, Keys["E"])  and GetVehicleEngineHealth(Vehicle.Vehicle) <= 100 then
