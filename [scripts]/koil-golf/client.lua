...
Citizen.CreateThread(function()
	...
	while true do
		...
		if distance < 1.5 then
			...
			if golfplaying then
				DisplayHelpText("Нажмите ~g~~INPUT_CONTEXT~~s~ чтобы завершить игру")
			else
				DisplayHelpText("Нажмите ~g~~INPUT_CONTEXT~~s~ чтобы начать игру, цена: $100")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					TriggerEvent("customNotification","Нажмите E для удара, A-D для поворота, Y для смены клуба.")
				end
			end
...
function startGolfHud()
	while golfplaying do
		...
		if golfhole ~= 0 then
			...
			drawGolfTxt(0.9193, 1.391, 1.0,1.0,0.4, "~s~" .. golfstrokes .. "~r~ - ~s~" .. totalgolfstrokes .. "~r~ - ~s~" .. clubname .. "~r~ - ~s~" .. distance .. " м", 255, 255, 255, 255)
		end
	end
end
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Переместитесь к мячу, нажмите E, чтобы сбросить мяч, если вы застряли.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	if distance < 1.5 then
		TriggerEvent("customNotification","Вы получили мяч!")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		TriggerEvent("customNotification","Вы сделали слишком много ударов")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("Нажмите ~g~E~s~ для броска здесь.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("Нажмите ~g~E~s~ для броска - ~r~ слишком далеко от мяча или близко к отверстию.")
		end
	end
end
...
function idleShot()
	Citizen.Trace("Пробный удар включен")
	power = 0.1
...
AddEventHandler('customNotification', function(response)
	TriggerEvent('chatMessage', 'ГОЛЬФ: ', { 0, 11, 0 }, response)
end)
