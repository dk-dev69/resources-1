...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(isNear(tenue)) then
				Info("Нажмите ~g~E~w~ чтобы изменить.")
		...
		if(isNear(gamestart)) then
				Info("Нажмите ~g~E~w~ чтобы начать игру.")
				if(IsControlJustPressed(1, 38)) then
					debutpartie()
				end
		end
		...
		if(isNear(miseazero)) then
				Info("Нажмите ~g~E~w~ чтобы обнулить счет до нуля.")
				if(IsControlJustPressed(1, 38)) then
					remettrezero()
				end
		end
		...
		if (Vdist(pos.x, pos.y, pos.z, home.x, home.y, home.z) < 2.0) then
				Info("Нажмите ~g~E~w~ чтобы играть в пейнтбол.")
				if(IsControlJustPressed(1, 38)) then
					allerjouer()
				end
		end
		...
		if (Vdist(pos.x, pos.y, pos.z, activity.x, activity.y, activity.z) < 2.0) then
				Info("Нажмите ~g~E~w~ выйти из игры.")
				if(IsControlJustPressed(1, 38)) then
					quitterpaintball()
				end
		end
...
function OuvrirVestiaire()
  ...
  local elements = {
    { label = ('Гражданская одежда'), value = 'citizen_wear' }
  }


    table.insert(elements, {label = "Одежда первой команды", value = 'tenue_equipe'})
	table.insert(elements, {label = "Одежда второй команды", value = 'tenue_equipedeux'})
...
function debutpartie()
	...
	if(distancenotification(millieu)) then
		TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 100, "countdown", 0.3)
		ESX.ShowNotification('~p~Игра начнется ~r~~h~3')
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~Игра начнется ~r~~h~2')
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~Игра начнется ~r~~h~1')
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~Игра ~r~~h~началась!')
	end
end
...
Citizen.CreateThread(function()
    function Initialize(scaleform)
        ...
        PushScaleformMovieFunctionParameterString("~r~Хороший пейнтбол!!")
		PushScaleformMovieFunctionParameterString("Приготовьте ваше оружие!")
...
ESX.ShowNotification('~p~Счет обнулен')
end
