...
local speed_measure = "км/ч" -- default unit to measure vehicle speed but can be changed to "MPH". Use either exact string, "Km/h" or "MPH", or else functions break.
...
function RenderVehicleInfo(vehicle)
	if DoesEntityExist(vehicle) then
		...
		SetTextEntry("STRING")
		if vehicle_display == 0 then
			AddTextComponentString("Скорость: " .. math.ceil(vehspeed) .. " " .. speed_measure .. "\nМодель: " .. vehname .. "\nНомер: " .. licenseplate)
		elseif vehicle_display == 1 then
			AddTextComponentString("Модель: " .. vehname .. "\nНомер: " .. licenseplate)
		end
		DrawText(0.45, 0.9)
	end
end
