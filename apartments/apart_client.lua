...
local txt = {
  ...
  ['ru'] = {
        ...
        ['sonner'] = 'Позвоните в дверь',
        ['gohome'] = 'Перейти в апартаменты',
        ['vendre'] = 'Продать апартаменты',
        ['acheter'] = 'Купить апартаменты',
        ['visiter'] = 'Посетить апартаменты',
        ['menu'] = 'Нажмите ~g~E~s~ для открытия меню',
        ['soon'] = 'Эта функциональность скоро будет',
        ['exit'] = 'Выход',
        ['retirerargent'] = 'Взять деньги из апартаментов',
        ['retirersale'] = 'Взять грязные деньги',
        ['deposerargent'] = 'Оставить чистые деньги',
        ['deposersale'] = 'Оставить грязные деньги'
  },
    ...
    for i=1, #interiors do
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), interiors[i].xo,interiors[i].yo,interiors[i].zo, true) < 1.599 then
            ...
            if isBuy == 2 then
            	print(money)
            	print(dirtymoney)
                Menu.addButton("Чистые деньги: " .. money .. " $","privetimuseless",nil)
                Menu.addButton("Грязные деньги: " .. dirtymoney .." $","privetimuseless",nil)
    ...
    for i=1, #interiors do
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), interiors[i].xe,interiors[i].ye,interiors[i].ze, true) < 1.599 then
            ...
            elseif isBuy == 2 then
                ...
                Menu.addButton("Цена: " .. interiors[i].price .. " $","privetimuseless",nil)
