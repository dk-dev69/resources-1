...
--Продавцы в магазинах аммуниции стандартные
Citizen.CreateThread(function()
	...
	if Config.EnableShops or Config.EnableWeapons then
		for _, item in pairs(Config.Locations23) do
			local npc = CreatePed(4, 0x9E08633D, item.x, item.y, item.z, false, false)
			FreezeEntityPosition(npc, false)	
			SetEntityHeading(npc, item.heading)
			SetEntityInvincible(npc, false) --NPC can die (set "true" to change)
			SetBlockingOfNonTemporaryEvents(npc, false)
...
--Продавцы в магазинах аммуниции военные
Citizen.CreateThread(function()
	...
	if Config.EnableShops or Config.EnableWeapons then
		for _, item in pairs(Config.Locations24) do
			local npc = CreatePed(4, 0xB3F3EE34, item.x, item.y, item.z, false, false)
			FreezeEntityPosition(npc, false)	
			SetEntityHeading(npc, item.heading)
			SetEntityInvincible(npc, false) --NPC can die (set "true" to change)
			SetBlockingOfNonTemporaryEvents(npc, false)
...
--Персонал в различных сервисах
Citizen.CreateThread(function()
    RequestModel(GetHashKey("ig_molly"))
	
    while not HasModelLoaded(GetHashKey("ig_molly")) do
        Wait(1)
    end
	
		for _, item in pairs(Config.Locations25) do
			local npc = CreatePed(4, 0xAF03DDE1, item.x, item.y, item.z, false, false)
			FreezeEntityPosition(npc, false)	
			SetEntityHeading(npc, item.heading)
			SetEntityInvincible(npc, false) --NPC can die (set "true" to change)
			SetBlockingOfNonTemporaryEvents(npc, false)
...
local ShopClerk = {
{id = 1, name = "Shop Keeper", VoiceName = "SHOP_GREET", Ambiance = "AMMUCITY", modelHash = "mp_m_shopkeep_01", x = -641.33, y = -246.4, z = 38.2, heading = 305.56}
}
-- Some Vars
...
local JobSearchManager = {
{id = 1, name = "Administrator Job", VoiceName = "SHOP_GREET", Ambiance = "AMMUCITY", modelHash = "a_m_y_business_02", x = -351.96, y = -989.58, z = 32.25, heading = 254.47}
}
-- Some Vars
local generalLoaded = false
local PlayingAnim = false
...
Citizen.CreateThread(function()
  while true do
	...
	if (not generalLoaded) then
	  for i=1, #JobSearchManager do
        RequestModel(GetHashKey(JobSearchManager[i].modelHash)) -- Gets model hash from table we made earlier
        while not HasModelLoaded(GetHashKey(JobSearchManager[i].modelHash)) do
          Wait(1)
        end
		JobSearchManager[i].id = CreatePed(2, JobSearchManager[i].modelHash, JobSearchManager[i].x, JobSearchManager[i].y, JobSearchManager[i].z, JobSearchManager[i].heading, true, true)
		SetAmbientVoiceName(JobSearchManager[i].id, JobSearchManager[i].Ambiance) -- Sets Voice Type
        -- Add all the Ped preferences like health, etc.
      end
      generalLoaded = true
    end
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(0)
		...
		for i=1, #JobSearchManager do
			distance = GetDistanceBetweenCoords(JobSearchManager[i].x, JobSearchManager[i].y, JobSearchManager[i].z, GetEntityCoords(GetPlayerPed(-1)))
			if distance < 4 and PlayingAnim ~= true then
				TaskPlayAnim(JobSearchManager[i].id,"random@shop_gunstore","_greeting", 1.0, -1.0, 4000, 0, 1, true, true, true) -- Plays animation
				PlayAmbientSpeech1(JobSearchManager[i].id, JobSearchManager[i].VoiceName, "SPEECH_PARAMS_FORCE", 1) -- Makes Ped Talk from audio file we defined in the table up top
				PlayingAnim = true
				Citizen.Wait(4000)
				if PlayingAnim == true then
					TaskPlayAnim(JobSearchManager[i].id,"random@shop_gunstore","_idle_b", 1.0, -1.0, -1, 0, 1, true, true, true)
					Citizen.Wait(40000)
				end
			end
			if distance > 5.5 and distance < 6 then
