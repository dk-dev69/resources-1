...
function OpenBusinessMenu(business)
    ...
    ESX.TriggerServerCallback("esx_business:getStock", function(stock)
        local elements = {}
        table.insert(elements,{label = "Акции: "..tostring(stock), value = ''})
        table.insert(elements,{label = 'Купить акции - <span style="color:green;">$'..tostring(business["stock_price"])..'</span>', label_real = 'buystock', value = 1, type = 'slider', min = 1, max = 100})
        if business["owner"]==PlayerData.identifier then table.insert(elements,{label = 'Продать бизнес - <span style="color:red;">$'..tostring(math.floor(business["price"]*Config.sell_percentage))..'</span>', value = 'sellbusiness'}) end
        if business["owner"]==PlayerData.identifier then table.insert(elements,{label = 'Список сотрудников', value = 'employeelist'}) end
        table.insert(elements,{label = "Выход из меню", value = 'exitmenu'})
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_boss',
        {
            ...
            if data.current.label_real == 'buystock' then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_confirm_stock', {
                    title    = ("Вы уверены, что хотите купить акции %s за %s?"):format(data.current.value,business["stock_price"]*data.current.value),
                    align    = 'bottom-right',
                    elements = {
                        {label = 'Нет',  value = 'no'},
                        {label = 'Да', value = 'yes'},
                    ...
                    if data2.current.value == 'yes' then
                        ESX.TriggerServerCallback("esx_business:buyStock", function(success,time)
                            if success then
                                ESX.ShowNotification("Вы успешно купили акции, они станут доступны "..disp_time(time))
                            else ESX.ShowNotification("~r~Вам не хватает денег") end
            ...
            elseif data.current.value == 'sellbusiness' then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_confirm_sellbusiness', {
                    title    = ("Вы уверены, что хотите продать %s за %s?"):format(business["name"],math.floor(business["price"]*Config.sell_percentage)),
                    align    = 'bottom-right',
                    elements = {
                        {label = 'Нет',  value = 'no'},
                        {label = 'Да', value = 'yes'},
                    ...
                    if data2.current.value == 'yes' then
                        ESX.TriggerServerCallback("esx_business:sellBusiness", function(success)
                            if success then
                                ESX.ShowNotification("Вы продали свой бизнес за ~g~$"..math.floor(business["price"]*Config.sell_percentage))
                            else ESX.ShowNotification("~r~Произошла ошибка") end
                        end,business["id"])
                    end
                    menu2.close()
                end, function(data2, menu2) menu2.close() end)
            ...
            elseif data.current.value=="employeelist" then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_choice', {
                    title    = business["name"]..' - сотрудники',
                    align    = 'bottom-right',
                    elements = {
                        {label = 'Нанять сотрудника',  value = 'hire'},
                        {label = 'Уволить сотрудника', value = 'fire'},
                    }
                }, function(data2, menu2)
                    if data2.current.value=="hire" then
                        ESX.TriggerServerCallback("esx_business:getNewEmployeeList",function(newemployees)
                            ...
                            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_hire', {
                                title    = business["name"]..' - Нанять сотрудника',
                                align    = 'bottom-right',
                                elements = newemployeeselements
                            }, function(data3, menu3)
                                ESX.TriggerServerCallback("esx_business:hireEmployee",function(isok)
                                    ESX.ShowNotification(isok and "~g~Сотрудник успешно принят" or "~r~произошла ошибка")
                    ...
                    elseif data2.current.value=="fire" then
                        ESX.TriggerServerCallback("esx_business:getEmployeeList",function(employees)
                            ...
                            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_fire', {
                                title    = business["name"]..' - уволить сотрудника',
                                align    = 'bottom-right',
                                elements = employeeselements
                            }, function(data3, menu3)
                                ESX.TriggerServerCallback("esx_business:fireEmployee",function(isok)
                                    ESX.ShowNotification(isok and "~g~Сотрудник успешно уволен" or "~r~произошла ошибка")
...
function OpenBuyBusinessMenu(business)
    ...
    {
        title = "Купить "..business["name"].." за $"..business["price"].."?",
        align = 'bottom-right',
        elements = {
            {label = 'Да', value = 'yes'},
            {label = 'Нет', value = 'no'}
        ...
        if data.current.value == 'yes' then
            ESX.TriggerServerCallback("esx_business:buyBusiness",function(status)
                if status==0 then
                    ESX.ShowNotification("Вы успешно купили ~b~"..business["name"].."~s~!")
                elseif status==1 then
                    ESX.ShowNotification("~r~"..business["name"].."~s~ уже есть владелец!")
                else
                    ESX.ShowNotification("У вас недостаточно денег, чтобы купить ~b~"..business["name"].."~s~!")
                end
            end,business["id"])
        end
...
function OpenCreateBusinessMenu()
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_create',
    {
        ...
        elements = {
            ...
            {label = '<span style="color:red;">Налоговая ставка % 0.01=1% - 0.1=10% - 1.0=100%</span>', action = ''},
            ...
            if GetDistanceBetweenCoords(ppos,x,y,z,false)<Config.draw_distance then
                if business.owner==nil then
                    if GetDistanceBetweenCoords(ppos,x,y,z,false)<2.0 and not buy then
                        ESX.ShowHelpNotification('Нажмите ~INPUT_CONTEXT~ чтобы купить бизнес')
                ...
                elseif business.owner==PlayerData.identifier or business.isemployee then
                    ...
                    if GetDistanceBetweenCoords(vector3(xx,yy,zz),GetEntityCoords(ped),false)<1.0 and not boss then
                        ESX.ShowHelpNotification('Нажмите ~INPUT_CONTEXT~ чтобы открыть меню')
