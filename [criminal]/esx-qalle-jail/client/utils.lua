RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job ~= nil and (PlayerData.job.name == "police" or PlayerData.job.name == "fbi") then
		...
		ESX.ShowNotification("У вас нет доступа!")
	end
end)
...
--  female
	...
	RemoveAllPedWeapons(PlayerPed, true)--удаляем все оружие у заключенных
...
function TeleportPlayer(pos)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Тюремная охрана" then
				if PlayerData.job.name ~= "police" or PlayerData.job.name ~= "fbi" then
					ESX.ShowNotification("У вас ~r~нет доступа")
					return
				end
			end
...
Citizen.CreateThread(function()
    ...
    AddTextComponentString('Тюрьма Болингброк')
