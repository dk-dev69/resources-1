		...
		if IsControlJustPressed(0, 51) and not onMission and IsPedInAnyVehicle(PlayerPedId()) then
			...
			if IsMissionVehicle(GetEntityModel(veh)) then
				...
				if time >= 0 and time <= TimeToSeconds(5, 30, 0) then
					...
					ShowMPMessage("Burglary", "Найдите ~r~дом ~s~ для ограбления", 4500)
					--ShowSubtitle("Find a ~r~house ~s~ to rob")
				else
					DisplayHelpText("Кражи со взломом можно осуществить только с 0:00 до 5:30 утра.")
				end
			end
		end
		...
		if onMission then		
			for k,door in pairs(doors) do
				...
				if Vdist(coords, door.coords.x, door.coords.y, door.coords.z) < 2 then
					DisplayHelpText("Нажмите ~INPUT_CONTEXT~ чтобы войти в дом.")
					...
					if IsControlJustPressed(0, 51) then
						...
						ShowSubtitle("Вы находитесь в чужом доме, будьте осторожны")
						ShowSubtitle("Чтобы не делать слишком много шума. (Крадитесь)")
