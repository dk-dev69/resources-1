		...
		if isHolding then
			...
			if IsControlJustPressed(0, 51) then	
				...
				if isInHouse then
					...
					if Vdist(markerCoords, coords) < 3 then
						ShowSubtitle("Предмет в багажнике, возвращайтесь внутрь, чтобы украсть больше предметов.")
			...
			for _,pickup in pairs(pickups) do
				...
				if onMission and Vdist(coords, GetEntityCoords(pickup.prop)) < 2 and not isHolding then
					DisplayHelpText("Нажмите ~INPUT_CONTEXT~ чтобы украсть предмет")
					...
					if IsControlJustPressed(0, 51) then
						...
						ShowSubtitle("Вы подняли предмет, отнесите его в фургон снаружи.")
