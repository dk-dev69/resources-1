...
Citizen.CreateThread(function()
		...
		AddTextComponentString("Контрабанда")
...
Citizen.CreateThread(function()
	...
	Wait(1)
		...
		SetEntityInvincible(meth_dealer_seller, false)
		FreezeEntityPosition(meth_dealer_seller, false)
	...
	while true do
		...
		local v = Config.CargoProviderLocation 
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Нажмите ~INPUT_CONTEXT~ для взаимодействия с ~y~дилером")
			...
			if event_is_running then
				...
				if pVehicle == event_vehicle then
					...
					if delivery_point_distance < 50.0 then
					 ...
					 DrawMissionText("Вернись в машину", 1000)
				end
			end
	end
end)
		...
		Citizen.Wait(5000)
				if event_is_running then
						...
						if IsPedDeadOrDying(GetPlayerPed(-1)) then
							ResetCargo()
							DisplayMissionFailed('Вы погибли, мисси провалена')
						end

						if GetVehicleEngineHealth(event_vehicle) < 20 and event_vehicle ~= nil then
							ResetCargo()
							DisplayMissionFailed('Груз поврежден, мисси провалена')
						end

						if event_time_passed > 1800 then
							ResetCargo()
							DisplayMissionFailed('Время доставки груза истекло')
						end
...
function CargoMenu()
	ClearMenu()
    options.menu_title = "Дилер"
    for i = 1, #Config.Scenarios do
    	Menu.addButton("Купить груз - за $" .. Config.Scenarios[i].CargoCost, "PurchaseCargo", i)
   	end
    Menu.addButton("Закрыть","CloseMenu",nil) 
end
...
function AlertThePolice()
	...
	if event_is_running then
        ...
        TriggerEvent('esx_phone:registerNumber', 'police', "Предположительно транспорт с номером [ " .. vehicle_plate .. " ] перевозит контрабанду", true, true)
		
    	TriggerServerEvent('esx_addons_gcphone:startCall', 'police', "Предположительно транспорт с номером [ " .. vehicle_plate .. " ] перевозит контрабанду", PlayerCoords, {
    	...
    	for i = 1, #Config.AlertExtraSocieties do
    		...
    		if PlayerData.job.name ~= Config.AlertExtraSocieties[i] then
    			...
    			TriggerServerEvent('esx_addons_gcphone:startCall', Config.AlertExtraSocieties[i], "Предположительно транспорт с номером [ " .. vehicle_plate .. " ] перевозит контрабанду", PlayerCoords, {
				PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z },
				})
...
function PurchaseCargo(scenario)
	...
	if event_is_running == true then
		...
		drawNotification("Вы уже выполняете доставку.")
		goto done

	end

	print("Необходимо минимум полицейских онлайн: " .. Config.Scenarios[scenario].MinCopsOnline .. " ||  Стоимость груза:  " .. Config.Scenarios[scenario].CargoCost .. "")
	...
	ESX.TriggerServerCallback('esx_cargodelivery:getCopsOnline', function(police)
		...
		if police >= Config.Scenarios[scenario].MinCopsOnline then
			...
			if bought then
				...
				drawNotification("Груз успешно куплен.")
			...
			drawNotification("Нужно как минимум ~b~" .. Config.Scenarios[scenario].MinCopsOnline .. " полицейских ~w~онлайн.")
