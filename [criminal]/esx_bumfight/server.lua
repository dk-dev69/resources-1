        ...
        while waitPlayerReady do
            ...
            elseif (bluePlayer ~= nil) and (redPlayer ~= nil) and (queuePlayer == nil) then
                if (_source ~= bluePlayer) and (_source ~= redPlayer) then
                    queuePlayer = _source
                    TriggerClientEvent('esx:showNotification', queuePlayer, 'Подожди, когда закончится ~y~бой~w~, ~g~ты следующий~w~!')
                end
            elseif (bluePlayer ~= nil) and (redPlayer ~= nil) and (queuePlayer ~= nil) then
                if (_source ~= bluePlayer) and (_source ~= redPlayer) and (_source ~= queuePlayer) then
                    TriggerClientEvent('esx:showNotification', _source, "~r~Бойцов уже достаточно~w~, возвращайся к следующему ~y~бою~w~!")
                end
...
function nextBlue()
    if redPlayer ~= nil then
        ...
        if queuePlayer ~= nil then
            ...
            TriggerClientEvent('esx:showNotification', redPlayer, '~r~Никто не хочет драться с тобой!')
...
function nextRed()
    if bluePlayer ~= nil then
        ...
        if queuePlayer ~= nil then
            ...
            TriggerClientEvent('esx:showNotification', bluePlayer, '~r~Никто не хочет драться с тобой!')
    ...
    function(betAmount, betOnFighter)
        ...
        if (Bets[bettorName] == nil) then
            ...
            if betOnFighter == 'blue' then
                ...
                TriggerClientEvent('esx:showNotification', _source, 'Ваша ~g~ставка~w~ была принята!')
                TriggerClientEvent('esx:showNotification', bluePlayer, 'На вас ~g~поставили~w~ не подведите их!')
            ...
            elseif betOnFighter == 'red' then
                ...
                registerBet(bettorName, _source, betWin, payement, score, betOnFighter)
                TriggerClientEvent('esx:showNotification', _source, 'Ваша ~g~ставка~w~ была принята!!')
                TriggerClientEvent('esx:showNotification', redPlayer, 'На вас ~g~поставили~w~ не подведите их!')
				xPlayer.removeMoney(betAmount)
            end
        elseif (Bets[bettorName] ~= nil) and (Bets[bettorName].bet == 0) then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~r~Вы уже сделали ставку!')
        end
    end
)
...
AddEventHandler(
    ...
    function()
        ...
        if Bets[sourceName] == nil then
            TriggerClientEvent('esx:showNotification', _source, '~r~Вы не делали ставки')
        end
        for i, val in pairs(Bets) do
            if val.player == _source then
                if val.score <= 3 then
                    TriggerClientEvent('esx:showNotification', _source, 'Вы выиграли ' .. val.payement .. '$')
                    xPlayer.addAccountMoney('black_money', val.payement)
                else
                    TriggerClientEvent('esx:showNotification', _source, '~r~Вы пытаетесь разоблачить меня?')
                    TriggerClientEvent('bumfight:callGang')
                    val.wanted = true
                end
                val.payement = 0
            elseif val.payement == 0 then
                TriggerClientEvent('esx:showNotification', _source, '~r~У вас нет ставок')
            end
        end
    end
)
...
function blueReward()
    ...
    if totalBetOnBlue > 0 then
        ...
        TriggerClientEvent('esx:showNotification', bluePlayer, "Вы выиграли ~g~"..rewardBlue.."~w~ $")
    else
        TriggerClientEvent('esx:showNotification', bluePlayer, "На вас не сделали ставок, ~r~вы ничего не выиграли!")
    end
end
...
function redReward()
    ...
    if totalBetOnRed > 0 then
        ...
        TriggerClientEvent('esx:showNotification', redPlayer, "Вы выиграли ~g~"..rewardRed.."~w~ $")
    else
        TriggerClientEvent('esx:showNotification', redPlayer, "На вас не сделали ставок, ~r~вы ничего не выиграли!")
    end
end