			...
			if(Vdist(x,y,z,robbableSpot.x,robbableSpot.y,robbableSpot.z)<distanceForMarkerToShow and isRobbing==false and robbableSpot.beingRobbed==false)then
				...
				if(Vdist(x,y,z,robbableSpot.x,robbableSpot.y,robbableSpot.z)<distanceToInteractWithMarker)then
					...
					if(robbableSpot.isSafe)then
						DisplayHelpText("Нажмите ~INPUT_CONTEXT~ чтобы взломать сейф.")
					else
						DisplayHelpText("Нажмите ~INPUT_CONTEXT~ чтобы открыть кассу.")
					end
		...
		Citizen.CreateThread(function()
			while isRobbing do
				if(spotBeingRobbed.isSafe)then
					DisplayHelpText("Взлом сейфа, осталось ("..spotBeingRobbed.timeToRob-currentSecondCount.." сек.)")
				else
					DisplayHelpText("Взлом кассового аппарата, осталось ("..spotBeingRobbed.timeToRob-currentSecondCount.." сек.)")
				end
				Citizen.Wait(0)
			end
		end)
...
function RobberyOver()
	DisplayHelpText("Вы успешно произвели ограбление!")
...
AddEventHandler("robberies:robberyOverNotification", function(name)
	TriggerEvent("pNotify:SendNotification", {
		text = "10-90: Кто-то видел покидающих район ( "..name.." )",
		type = "warning",
		timeout = 20000,
		layout = "centerLeft",
		queue = "right"
...
AddEventHandler("robberies:robberyStartedNotification", function(name)
	TriggerEvent("pNotify:SendNotification", {
		text = "10-90 : Сработала тревожная кнопка ( "..name.." )",
		type = "warning",
		timeout = 20000,
		layout = "centerLeft",
		queue = "right"
