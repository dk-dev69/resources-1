  ...
  while true do 
    Citizen.Wait(1)
      ...
      if hack_distance <=1 and not IsEntityDead(ped) and not hacking then
        drawTxt("Нажмите ~b~E~w~ для взлома системы",4,0.5,0.93,0.50,255,255,255,180)
...
function mycb(success, timeremaining)
  if success then
    local ped = PlayerPedId()
    TriggerEvent('chatMessage', "Вы успешно взломали систему, информация о транспорте доступна на карте.")
  ...
  for i=0, 1, 1 do
    ...
    if GetVehicleDoorAngleRatio(variavel, 2) > .1 or GetVehicleDoorAngleRatio(variavel, 3) > .1 and flag == 0 then
      ...
      money_carf = CreateAmbientPickup(GetHashKey("PICKUP_MONEY_SECURITY_CASE"), GetOffsetFromEntityInWorldCoords(variavel, 0.0, -5.0, -0.3001), 0, 300000) 
        --cashPickup = CreateAmbientPickup(GetHashKey("PICKUP_MONEY_SECURITY_CASE"), GetOffsetFromEntityInWorldCoords(variavel, 0.0, -5.0, -0.3001), 0, 300000)
		--money_carf = CreateAmbientPickup(GetHashKey("PICKUP_MONEY_SECURITY_CASE"), GetOffsetFromEntityInWorldCoords(variavel, 0.0, -5.0, -0.3001), 0, 30000)
		---CreateAmbientPickup(GetHashKey("PICKUP_MONEY_SECURITY_CASE"), GetOffsetFromEntityInWorldCoords(variavel, 0.0, -5.0, -0.3001), 0, 40000, 1, false, true)
		--cashPickup = CreatePickup(GetHashKey("PICKUP_MONEY_SECURITY_CASE"), GetOffsetFromEntityInWorldCoords(variavel, 0.0, -5.0, -0.3001))
	
		--if HasPickupBeenCollected(cashPickup) then
		--	 TriggerServerEvent('securityvans:getmoney')
