...
function OpenTruckShop(shop)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'truck_shop',
	{
		title    = _U('truck_shop'),
		align    = 'top-left',
		elements = elements
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'truck_shop_confirm',
		{
			title    = _U('truck_shop_confirm', data.current.name, data.current.price),
			align    = 'top-left',
			elements = {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_truck:buyTruck', function (bought)

					if bought then
						ESX.ShowNotification(_U('truck_shop_bought', data.current.name, data.current.price))
						...
						ESX.ShowNotification(_U('truck_shop_nomoney'))
						menu2.close()
					end
...
function OpenTruckGarage(garage)

	ESX.TriggerServerCallback('esx_truck:getGarage', function (ownedTrucks)

		if #ownedTrucks == 0 then
			ESX.ShowNotification(_U('garage_notrucks'))
		else

			-- get all available trucks
			local elements = {}
			for i=1, #ownedTrucks, 1 do
				table.insert(elements, {
					label        = getVehicleLabelFromHash(ownedTrucks[i].model),
					vehicleProps = ownedTrucks[i]
				})
			end

			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'truck_garage',
			{
				title    = _U('garage'),
				align    = 'top-left',
				elements = elements
				...
				if ESX.Game.IsSpawnPointClear(garage.SpawnPoint, 4.0) then
					TriggerServerEvent('esx_truck:takeOutVehicle', vehicleProps.plate)
...
function OpenLicenceMenu(shop)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'truck_license',
	{
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('esx_truck:buyTruckLicense', function (boughtLicense)
				if boughtLicense then
					ESX.ShowNotification(_U('license_bought', Config.LicensePrice))
					menu.close()

					OpenTruckShop(shop) -- parse current shop
				else
...
function StoreTruckInGarage(vehicle, teleportCoords)

	local vehicleProps = ESX.Game.GetVehicleProperties(vehicle)

	ESX.TriggerServerCallback('esx_truck:storeVehicle', function (rowsChanged)
