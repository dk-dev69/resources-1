(function(){
	...
	let renderContacts = function(){
		...
		if(contacts.length > 0) {
			...
			contactHTML = '<div class="contact no-item online"><p class="no-item">Нет контактов</p></div>';
		}
	...
	let addMessage = function(phoneNumber, pmessage, pposition, panonyme, pjob){
		...
		if(messages.length >  0) {
			...
			messageHTML = '<div class="message no-item"><p class="no-item">Нет сообщений</p></div>';
		}
	...
	let renderSpecialContacts = function(){
		...
		$('#phone .menu .home').html(
			'<li class="phone-icon" id="phone-icon-rep">Контакты</li>' +
			'<li class="phone-icon" id="phone-icon-msg">Сообщения</li>'
		);
