...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    { label = ('Patron Kıyafeti'),    value = 'patron_outfit'},
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'patron_outfit' then
        setUniform(data.current.value, playerPed)
        isBarman = true
        TriggerServerEvent("player:serviceOn", "unicorn")
      end
...
function OpenSocietyActionsMenu()
  ...
  table.insert(elements, {label = ('DJ Çalıştır'),    value = 'hakanonur1'})
  table.insert(elements, {label = ('DJ Kapat'),    value = 'hakanonur2'})
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'hakanonur1' then
        TriggerEvent('esx_jb_dj:enabledjbooth', true)
       ESX.ShowNotification('DJ Bölümü aktif')
      
      end

      if data.current.value == 'hakanonur2' then
        TriggerEvent('esx_jb_dj:enabledjbooth', false)
       ESX.ShowNotification('DJ Bölümü kapatıldı')
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'bank_account', _U('billing'), amount)
        end
...
Citizen.CreateThread(function()
    ...
    local Flacons = Config.Blips.Flacons
    local deneme2 = AddBlipForCoord(Flacons.Pos.x, Flacons.Pos.y, Flacons.Pos.z)

    SetBlipSprite (deneme2, Flacons.Sprite)
    SetBlipDisplay(deneme2, Flacons.Display)
    SetBlipScale  (deneme2, Flacons.Scale)
    SetBlipColour (deneme2, Flacons.Colour)
    SetBlipAsShortRange(deneme2, true)

    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString('Pavyon Toptancı')
    EndTextCommandSetBlipName(deneme2)
