...
local blip = AddBlipForCoord(Config.SellFish.x, Config.SellFish.y, Config.SellFish.z)
			...
			AddTextComponentString("Продажа рыбы")
...
local blip2 = AddBlipForCoord(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Диллер Морская черепаха")
...
local blip3 = AddBlipForCoord(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Торговец мясом акулы")
...
for _, info in pairs(Config.MarkerZones) do
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Прокат лодок")
	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Keys['X']) then
				fishing = false
				ESX.ShowNotification("~r~Рыбалка завершена")
			end
			if fishing then
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					ESX.ShowNotification("~r~Рыба сорвалась")
				end
			end
		end
	...
	while true do
		...
		Wait(wait)
			if fishing then
				...
				ESX.ShowNotification("~g~Рыба взяла наживку \n ~h~Жмите " .. correct .. " чтобы подсечь")
				input = 0
				pausetimer = 0
			end
...
AddEventHandler('fishing:fishstart', function()
	...
	playerPed = GetPlayerPed(-1)
	local pos = GetEntityCoords(GetPlayerPed(-1))
	print('Рыбалка началась' .. pos)
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("~y~Вы не можете ловить рыбу из автомобиля")
	else
		if pos.y >= 7700 or pos.y <= -4000 or pos.x <= -3700 or pos.x >= 4300 then
			ESX.ShowNotification("~g~Рыбалка началась")
			ESX.ShowNotification("~r~Для завершения нажмите X")
			TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_STAND_FISHING", 0, true)
			fishing = true
		else
			ESX.ShowNotification("~y~Вы должны быть в зоне рыбалки\nОтмеченной на карте крючком")
		end
	end
    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
					...
					DisplayHelpText('Нажмите E, чтобы арендовать лодку')
...
function OpenBoatsMenu(x, y , z)
	...
	local elements = {}
		...
		table.insert(elements, {label = '<span style="color:green;">Надувной моторный катер</span> <span style="color:red;">2500$</span>', value = 'boat'})
		table.insert(elements, {label = '<span style="color:green;">Suntrap</span> <span style="color:red;">3500$</span>', value = 'boat6'}) 
		table.insert(elements, {label = '<span style="color:green;">Моторный катер</span> <span style="color:red;">4500$</span>', value = 'boat5'}) 	
		...
		table.insert(elements, {label = '<span style="color:green;">Баржа</span> <span style="color:red;">7500$</span>', value = 'boat4'})
		
	--If user has police job they will be able to get free Police Predator boat
	if PlayerData.job.name == "police" then
		table.insert(elements, {label = '<span style="color:green;">Полицеская лодка</span>', value = 'police'})
	end
	...
	if data.current.value == 'boat' then
		...
		TriggerEvent("chatMessage", 'Вы арендовали лодку за ', {255,0,255}, '$' .. 2500)
	...
	if data.current.value == 'boat2' then
		...
		TriggerEvent("chatMessage", 'Вы арендовали лодку за ', {255,0,255}, '$' .. 5500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat3' then
		...
		TriggerEvent("chatMessage", 'Вы арендовали лодку за ', {255,0,255}, '$' .. 6000)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat4' then
		...
		TriggerEvent("chatMessage", 'Вы арендовали лодку за ', {255,0,255}, '$' .. 7500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat5' then
		...
		TriggerEvent("chatMessage", 'Вы арендовали лодку за ', {255,0,255}, '$' .. 4500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat6' then
		...
		TriggerEvent("chatMessage", 'Вы арендовали лодку за ', {255,0,255}, '$' .. 3500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
