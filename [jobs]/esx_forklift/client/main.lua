...
function refreshBlips()
	if playerData.job.name ~= nil and playerData.job.name == 'fork' then 	
		drawBlip(Config.locker, 280, "Гардероб")			 	
		drawBlip(Config.carSpawner, 479, "Получить погрузчик")					
		drawBlip(Config.carDelete, 490, "Вернуть погрузчик")				
	end
end
...
function goDeliver()
	ESX.ShowNotification('Доставить ящик в отмеченное место.')
	setGPS(taskPoints['deliver'])
	currentJob = 'deliver'
end

function goPickup()
	ESX.ShowNotification('Езжайте к следующей позиции.')
...
function getPaid()
	...
	if IsPedInAnyVehicle(GetPlayerPed(-1)) and isMyCar() then			
		...
		ESX.ShowNotification('~w~Оплата за работу: ~g~ +' .. pay .. ' ~w~$.')
	else														
		ESX.ShowNotification('~r~Где погрузчик?')
		local pay = math.floor(packetsDelivered * (Config.pay * 0.6))			
		if packetsDelivered < 2 then						
			pay = 0								
		end
		TriggerServerEvent('esx_fork:getPaid', pay)
		ESX.ShowNotification('~w~Оплата за работу ~r~ +' .. amount .. ' ~w~$.')
	end
	currentJob = 'none'											
	currentPlate = ''												
	currentVehicle = nil											
	...
	while true do																
		...
		if not menuIsOpen then
			...
			elseif onDuty and currentJob == 'deliver' and taskPoints['deliver'] ~= nil and playerIsInside(playerCoords, taskPoints['deliver'], Config.pickupDistance) then
				isInMarker = true
				displayHint = true
				hintToDisplay = "Езжайте в отмеченное место."
				currentZone = 'none' 
			elseif playerData.job ~= nil and playerData.job.name == "fork" and currentPlate ~= '' and playerIsInside(playerCoords, Config.carDelete, 1.5) then  				
				isInMarker = true
				displayHint = true
				hintToDisplay = "Нажмите E, чтобы вернуть погрузчик"
				currentZone = 'pay'
			else																			
				isInMarker = false
				displayHint = false
...
function openMenu()									
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'locker',			
    {
      title    = "Гардероб",							
      elements = {
        {label = "Рабочая форма", value = 'fork_wear'},		
