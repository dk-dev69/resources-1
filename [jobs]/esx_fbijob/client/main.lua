...
function setUniform(job, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			...
			if job == 'special_forces' then
				 SetPedArmour(playerPed, 200)
				 local model = GetHashKey("s_m_y_noose_01")
				 RequestModel(model)
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = 'Экипировка спецназа', value = 'special_forces' }
	}

	if grade == 'recruit' then
		table.insert(elements, {label = _U('fbi_wear_recruit'), value = 'recruit_wear'})
	elseif grade == 'officer' then
		table.insert(elements, {label = _U('fbi_wear_officer'), value = 'officer_wear'})
	...
	elseif grade == 'boss' then
		table.insert(elements, {label = _U('fbi_wear_boss'), value = 'boss_wear'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if
			...
			data.current.value == 'special_forces'
		then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job and PlayerData.job.name == 'fbi' then
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job and PlayerData.job.name == 'fbi' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'fbi_actions') then
			if Config.MaxInService == -1 then
				OpenFBIActionsMenu()
			elseif playerInService then
				OpenFBIActionsMenu()
