             ...
             for i = 0, 31 do
				 if PlayerData.job ~= nil and PlayerData.job.name == 'police' or PlayerData.job.name == 'fbi' then
					 if DecorGetInt(GetPlayerPed(i), "IsOutlaw") == 2 and GetPlayerPed(i) ~= GetPlayerPed(-1) then
						 ...
						 Citizen.InvokeNative(0x613ED644950626AE, gamerTagId, 7, 1) --Белая звезда
					 elseif DecorGetInt(GetPlayerPed(i), "IsOutlaw") == 1 then
						 Citizen.InvokeNative(0x613ED644950626AE, gamerTagId, 7, 255) -- Белый цвет 255
						 Citizen.InvokeNative(0x63BB75ABEDC1F6A0, gamerTagId, 7, false) --Неактивный идентификатор игрока
					 end
				 end
             end
         end
...
Citizen.CreateThread( function()
    while true do
		...
		local x,y,z = -189.99, -893.46, 31.21
		---ставим кординаты центра города, от него будет отталкиваться на проверку стрельбы
		local dist = GetDistanceBetweenCoords(x, y, z, plyPos.x, plyPos.y, plyPos.z, true)
        ...
        if IsPedTryingToEnterALockedVehicle(GetPlayerPed(-1)) or IsPedJacking(GetPlayerPed(-1)) then
			...
			if (PlayerData.job ~= nil and showcopsmisbehave == false and (PlayerData.job.name == 'police' or PlayerData.job.name == 'fbi')) then
			elseif (PlayerData.job ~= nil and showcopsmisbehave and (PlayerData.job.name == 'police' or PlayerData.job.name == 'fbi')) then
...
--Не даем ботам ронять оружие из рук
