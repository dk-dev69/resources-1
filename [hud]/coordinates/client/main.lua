...
Citizen.CreateThread(function()
    while true do
		...
		Text("~r~X:~s~" .. math.ceil(playerPos.x) .." ~r~Y:~s~" .. math.ceil(playerPos.y) .." ~r~Z:~s~" .. math.ceil(playerPos.z) .." Heading: " .. math.ceil(playerHeading) .."")
	end
end)
...
Citizen.CreateThread(function()
    while true do
		...
		--DrawTxt("~r~Скорость: ~s~"..rounds, 0.40, 0.92)

		health = GetEntityHealth(PlayerPedId())
		--DrawTxt("~r~Здоровье: ~s~"..health, 0.40, 0.95)
		...
		if IsPedInAnyVehicle(PlayerPedId(), 1) then
			...
			--DrawTxt("~r~Состояние двигателя: ~s~"..vehenground, 0.015, 0.76)

			--DrawTxt("~r~Состояние кузова: ~s~"..vehbodround, 0.015, 0.73)

			DrawTxt("~r~Топливо: ~s~"..tonumber(string.format("%.2f", GetVehicleFuelLevel(GetVehiclePedIsUsing(PlayerPedId())))), 0.015, 0.70)
		end
    end
end)
