...
$(function() {
		window.addEventListener('message', function(event) {
			...
			if (event.data.owner == true) {
				...
				$('#wrapper').append(
					`
					<button class = "button" id = "bossactions" style = "position: absolute; right: 15px; top: 5px;">Меню владельца магазина</button>
					`
				);
			}
				...
				var i;
					for (i = 0; i < cb.length; i++) { 
						...
						$('#cart').append(
							...
							<input type="text" id = ${cb[i].item} count = ${cb[i].count} class = "textareas" placeholder = "Сколько?"></textarea>
							</div>`
								);
							};
							...
							$('#cart').append(
							`
							<button class = "button" id = "buybutton" style = "position: absolute; right: 15px; top: 5px;">Купить</button>
							<button class = "button" id = "back" style = "position: absolute; left: 15px; top: 5px;">Назад</button>
							`
						);
					});
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$.post('http://esx-kr-advanced-shops/notify', JSON.stringify({msg : "~r~Один товар не имеет достаточного запаса или сумма недействительна."}));
					}
				}
			});
