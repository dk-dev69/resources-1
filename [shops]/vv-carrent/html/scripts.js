...
$(".close").click(function(){
    $.post('http://vv_carrent/quit', JSON.stringify({}));
});
...
window.addEventListener('message', function (event) {
	...
	if (item.message == "add"){
		$( ".home" ).append('<div class="card">' +
			'<img src="img/' + item.item + '.jpg" alt="' + item.label + '" style="width:100%">' + 
			'<div class="container">' + 
				'<h4><b>' + item.label + '</b></h4> ' +
				'<div class="quantity">' + 
					'<div class="purchase">' + 
						'<div class="price">' + item.price + '$' + ' / Tag</div>' +
						'<div class="buy" name="' + item.item + '">Leasen</div>' + 
					'</div>' +
				'</div>' +
			'</div>');
...
$(".home").on("click", ".buy", function() {
	...
	$.post('http://vv_carrent/purchase', JSON.stringify({
		item: $name,
		loc: zone
	}));
});
