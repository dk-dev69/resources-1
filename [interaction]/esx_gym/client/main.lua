...
--local blips = {
--	{title="Спортзал", colour=7, id=311, x = -1201.2257, y = -1568.8670, z = 4.6101}
--}
...
AddEventHandler('esx_gym:useBandage', function()
	...
	ESX.ShowNotification("Вы использовали ~g~повязку")
end)
...
local arms = {
	...
	{x = 1644.04,y = 2533.28,z = 45.56}
}

local pushup = {
	{x = -1203.3242,y = -1570.6184,z = 4.6115},
	{x = 1637.94,y = 2524.85,z = 45.56},
...
local situps = {
	...
	{x = 1643.17,y = 2528.12,z = 45.56},
}

local gym = {
	{x = -1195.6551,y = -1577.7689,z = 4.6115},
	{x = 1645.11,y = 2539.18,z = 45.56},
}
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Нажмите ~INPUT_CONTEXT~, чтобы взять напрокат ~b~велосипед')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("У вас уже есть ~r~велосипед")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Нажмите ~INPUT_CONTEXT~ для доступа к ~b~меню~w~ спортзала')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Нажмите ~INPUT_CONTEXT~ чтобы начать тренировку ~g~рук')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Присутупаем к ~g~упражнению~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							exports["gamz-skillsystem"]:UpdateSkill("Выносливость", 0.3)--экспорт в система прокачки
							exports["gamz-skillsystem"]:UpdateSkill("Сила", 0.3)
							ESX.ShowNotification("Вам нужно отдохнуть ~w~прежде чем приступить к другому упражнению.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Вам нужно членство чтобы сделать ~r~упражнение")
						end
					elseif training == true then
						ESX.ShowNotification("Отдохните немного...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Нажмите ~INPUT_CONTEXT~ чтобы сделать несколько ~g~подтягиваний')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Подготовка к ~g~упражнению~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							exports["gamz-skillsystem"]:UpdateSkill("Выносливость", 0.3)--экспорт в система прокачки
							exports["gamz-skillsystem"]:UpdateSkill("Сила", 0.3)
							ESX.ShowNotification("Вам нужно отдохнуть ~r~10 сек. ~w~прежде чем делать другое упражнение.")
					...
					elseif training == true then
						ESX.ShowNotification("Вам нужно отдохнуть...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Нажмите ~INPUT_CONTEXT~ чтобы сделать ~g~отжимания')
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Теперь вы можете тренироваться снова ...")
	end
end
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Добро пожаловать! Мы открыты ~g~24~w~ часов/в сутки.")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Спорт товары',
            elements = {
				{label = 'Протеиновый коктейль ($40)', value = 'protein_shake'},
				{label = 'Вода ($15)', value = 'water'},
				{label = 'Спортивный ланч ($35)', value = 'sportlunch'},
				{label = 'Напиток Powerade ($50)', value = 'powerade'},
				{label = 'Повязка ($50)', value = 'bandage'},
            }
        },
        function(data, menu)
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Абонемент ($500)', value = 'membership'},
            }
        },
        function(data, menu)
...
function OpenBikeMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'bike_menu',
        {
            ...
            elements = {
				{label = 'BMX ($109)', value = 'bmx'},
				{label = 'Cruiser ($130)', value = 'cruiser'},
