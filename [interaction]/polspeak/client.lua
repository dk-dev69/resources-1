...
mainMenu = NativeUI.CreateMenu("POLICE MEGAPHONE", "~b~POLICE MEGAPHONE")
_menuPool:Add(mainMenu)

function AddMenuAnotherMenu(menu)
    local submenu = _menuPool:AddSubMenu(menu, "Остановить автомобиль", "Различные команды остановки автомобиля")
    local shout1 = NativeUI.CreateItem("LSPD! Стоп...", "LSPD! Остановите свой автомобиль сейчас!")
    local shout2 = NativeUI.CreateItem("Водитель! Стоп...", "Водитель! Останови свой автомобиль")
    local shout3 = NativeUI.CreateItem("Останови эту чертову машину ...", "LSPD! Немедленно останови эту чертову машину!")
    local shout4 = NativeUI.CreateItem("Останови или будете наказаны...", "LSPD! Остановите свой автомобиль сейчас, или вы будете казнены!")
    local shout5 = NativeUI.CreateItem("Останови или будем стрелять...", "Остановите свой автомобиль прямо сейчас, блядь! Или клянусь, я тебя убью!")
...
function Stop(menu)
    local submenu4 = _menuPool:AddSubMenu(menu, "Стоп »", "Различные команды остановки")
    local shout1 = NativeUI.CreateItem("Не заставляй меня...", "Стоп! Не заставляй меня стрелять в тебя! сдавайся!")
    local shout2 = NativeUI.CreateItem("Не двигайся...", "Stop and dont move a muscle, or you'll be shot by the LSPD!")
      ...
      _menuPool:ProcessMenus()
        ...
        if IsControlJustPressed(1, 57) then
          if vehicleType(GetVehiclePedIsUsing(GetPlayerPed(-1))) then
           ...
           ShowNotification("Вы ~r~не находитесь в транспортном средстве ~w~с мегафоном")
          end
        end
    end
end)
