...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 243)) then
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("Нет транспортных средств для блокировки поблизости.")
		else
			for j=1, #cars, 1 do
				...
				local distance = Vdist(coordscar.x, coordscar.y, coordscar.z, coords.x, coords.y, coords.z)
						-- RemoveBlip(copblip)
						-- copblip = AddBlipForCoord(coordscar.x, coordscar.y, coordscar.z)
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 7, "lock2", 0.2)
							ESX.ShowNotification('Сигнализация: ~y~'..vehicleLabel..' ~r~заблокирован~s~')
						...
						elseif lock == 2 then
							...
							TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 7, "lock2", 0.2)
							ESX.ShowNotification('Сигнализация: ~y~'..vehicleLabel..' ~g~разблокирован~s~ ')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
