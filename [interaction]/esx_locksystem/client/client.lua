...
AddEventHandler("esx_locksystem:newVehicle", function(plate, id, lockStatus)
    if(plate)then
        ...
        print("Не могу создать экземпляр транспортного средства. Недостающий аргумент PLATE")
    end
end)
...
function Notify(text, duration)
	if(Config.notification)then
		if(Config.notification == 1)then
			...
			Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_LIFEINVADER", "CHAR_LIFEINVADER", true, 1, "Все двери заблокированы", duration)
