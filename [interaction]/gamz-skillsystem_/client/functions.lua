...
UpdateSkill = function(skill, amount)
    ...
    if not Config.Skills[skill] then
        print("Навык " .. skill .. " не существуют")
        return
    end
    ...
    if Config.Notifications then
        if tonumber(amount) > 0 then
            ...
            ESX.ShowNotification("Ваш навык ~r~"..skill.." ~s~увеличился на ~g~+" .. amount .. "%")
        end
    end
...
RefreshSkills = function()
    for type, value in pairs(Config.Skills) do
        ...
        if value["Stat"] then
            ...
            StatSetInt(GetHashKey(value["Stat"]), round(value["Current"]), true)
