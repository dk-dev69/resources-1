...
AddEventHandler('pis:knifeCallout', function()
	location = "~o~24/7, Сэнди Шорс~w~"
	callout = "~r~Вооруженный человек~w~"
	response = "~r~Код 3~w~"
...
AddEventHandler('pis:abandonedVeh', function()
	location = "~o~Сэнди Шорс~w~"
	callout = "~r~Подозрительная машина~w~"
	response = "~r~Код 2~w~"
...
AddEventHandler('pis:shoplifting', function()
	location = "~o~24/7, Сэнди Шорс~w~"
	callout = "~r~Кража в магазине~w~"
	response = "~r~Код 2~w~"

	local ClerkPeaceQuotes = {"Вернитесь сюда!", "Офицер! вот этот парень", "Он взял что-то!"}
	local ClerkAgressiveQuotes = {"Вернитесь сюда!, ублюдок!!", "Офицер! вот эта сволочь", "That fucker took my shit!", "Motherfucker, get the fuck back!", "Shoot that bitch!", "I'm going to fucking kill him!"}
	...
	ShowNotification("Вероятно вооружен: " .. weaponChance .. " | AttitudeLvl: " .. AttitudeLvl)
	...
	Citizen.Wait(1000)
		...
		elseif distanceToClerk <= 5 and IsEntityDead(suspect) and HasSaidDeathQuote == false then
			...
			if ClerkSympathy == 1 then
				ShowNotification("~o~Клерк: ~w~" .. ClerkHeartlessQuotes[math.random(#ClerkHeartlessQuotes)])
			else
				ShowNotification("~o~Клерк: ~w~" ..  ClerkSympathyQuotes[math.random(#ClerkSympathyQuotes)])
			end
...
AddEventHandler('pis:fight', function()
	callID = "fight"
	location = "~o~Boat House, Сэнди Шорс~w~"
	callout = "~r~Драка~w~"
	response = "~r~Код 3~w~"
...
AddEventHandler('pis:shots', function()
	location = "~o~Cholla Springs Ave, Сэнди Шорс~w~"
	callout = "~r~Произведены выстрелы~w~"
	response = "~r~Коды 3~w~"
...
AddEventHandler('pis:crazy', function()
	location = "~o~Yellowjack, Сэнди Шорс~w~"
	callout = "~r~Произведены выстрелы~w~"
	response = "~r~Код 3~w~"
...
AddEventHandler('pis:weapon', function()
	...
	Citizen.Wait(1000)
		...
		if GetDistanceBetweenCoords(GetEntityCoords(sus1ab), GetEntityCoords(player)) < 10 and not IsPedInAnyVehicle(player) and not hasAimedWeapon and not hasMadeContact then
			ShowNotification("~o~Подозреваемый: ~w~" .. "Привет офицер.")
		...
		elseif DoesEntityExist(target) and target == sus1ab and not hasAimedWeapon then
			local suspectGunQuotes = {"Привет! Не указывайте, на меня!", "Эй! Убери этот пистолет от меня!", "В этом нет необходимости!"}
			ShowNotification("~o~Подозреваемый: ~w~" .. suspectGunQuotes[math.random(#suspectGunQuotes)])
			ClearPedTasks(sus1ab)
			TaskTurnPedToFaceEntity(sus1ab, player, -1)
			hasMadeContact = true
			hasAimedWeapon = true
...
AddEventHandler('pis:weapon:drop:q', function()
	...
	if speech == "Normal" then
		local officerDropQuotes = {"Брось это!", "Put the gun down!", "Come on, put it down.", "Work with me! Drop the gun!", "Брось оружие!"}
		ShowNotification("~o~Офицер: ~w~" .. officerDropQuotes[math.random(#officerDropQuotes)])
		suspectAttitude = suspectAttitude - math.random(5)
		retaliationChance = retaliationChance + math.random(6)
	else
		local officerDropQuotes = {"Черт, брось это!", "Put the fucking gun down!", "Put it the fuck down down.", "Work with me! Drop the gun!", "Drop the fucking weapon!"}
		ShowNotification("~o~Офицер: ~w~" .. officerDropQuotes[math.random(#officerDropQuotes)])
	...
	if suspectAttitude < 10 and not hasDroppedWeapon then
		...
		ShowNotification("~o~Подозреваемый: ~w~" .. suspectDropQuotes[math.random(#suspectDropQuotes)])
	...
	elseif hasDroppedWeapon then
		ShowNotification("~o~Подозреваемый: ~w~" .. "Я уже бросил!")
	end
end)
...
AddEventHandler('pis:weapon:face:q', function()
	...
	if speech == "Normal" then
		...
		ShowNotification("~o~Офицер: ~w~" .. officerFaceQuotes[math.random(#officerFaceQuotes)])
	else
		local officerFaceQuotes = {"Повернись бля!", "Отойди нахер от меня!"}
		ShowNotification("~o~Офицер: ~w~" .. officerFaceQuotes[math.random(#officerFaceQuotes)])
	end
	...
	if suspectAttitude < 10 and hasDroppedWeapon and not isFacingAway then
		local suspectFaceQuotes = {"Хорошо! Не стреляйте.", "Хорошо, я делаю это!", "Не стреляйте в меня! я сделаю это!", "Ладно ладно!"}
		ShowNotification("~o~Подозреваемый: ~w~" .. suspectFaceQuotes[math.random(#suspectFaceQuotes)])
	...
	elseif suspectAttitude > 10 and suspectAttitude < 95 and hasDroppedWeapon and not isFacingAway then
		local suspectFaceQuotes = {"Это неконституционно!", "I want your badge number!", "You're getting fired!", "You're breaching my rights!"}
		ShowNotification("~o~Подозреваемый: ~w~" .. suspectFaceQuotes[math.random(#suspectFaceQuotes)])
		TaskAchieveHeading(sus1ab, GetEntityHeading(player), 5000)
	else
		SetRelationshipBetweenGroups(5, GetHashKey("suspect"), GetHashKey("PLAYER"))
...
AddEventHandler('pis:weapon:threat:q', function()	
	...
	if speech == "Normal" then
		...
		ShowNotification("~o~Офицер: ~w~" .. officerThreatQuotes[math.random(#officerThreatQuotes)])
...
AddEventHandler('pis:weapon:knees:q', function()
	if speech == "Normal" then
		ShowNotification("~o~Офицер: ~w~" .. "Down on your knees!")
	else
		ShowNotification("~o~Офицер: ~w~" .. "Get the fuck down!")
	end
