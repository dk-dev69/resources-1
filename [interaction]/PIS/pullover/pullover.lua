				...
				if GetVehicleClass(playerVeh) == 18 then
					...
					elseif mimicking then
						...
						if IsVehicleSeatFree(targetVeh,-1) and IsVehicleSeatFree(targetVeh,0) or DoesEntityExist(targetVeh) == false then
							...
							ShowNotification( "Транспорт не обнаружен." )
						else
...
AddEventHandler('pis:follow', function()
	if stopped then
		...
		if following then
			...
			--(You need to pull over a vehicle first)
			ShowNotification( "Вы должны сначала сделать пуловер." )
		end
	end
end, false)
...
AddEventHandler('po:follow', function()
	...
	local playerVeh = GetVehiclePedIsIn(player, false)
		if DoesEntityExist(stoppedVeh) then
			ShowNotification("~r~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(stoppedVeh))) .. " ~w~следует за вами.")
			following = true
			Citizen.Wait(10)
			while (following) do 
				Citizen.Wait(1000)
...
AddEventHandler('po:unfollow', function()
	following = false
	Citizen.Wait(100)
	ShowNotification("~r~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(stoppedVeh))) .. " ~w~больше не следует за вами.")
	stopped = true
	lockedin = false
	Wait(2000)
	SetPedIntoVehicle(stoppedDriver, stoppedVeh, -1)
...
AddEventHandler('po:pullover', function()
	...
	cannabis = "~g~отрицательный"
	cocaine = "~g~отрицательный"
	if drugnum_cannabis > 85 then
		cannabis = "~r~положительный"
		drunk = true
		chanceFlee = math.random(18, 30)
	end
	if drugnum_cocaine > 90 then
		cocaine = "~r~положительный"
		drunk = true
...
RegisterCommand("runplate", function(s,args,raw)
	...
	if args[1] == vehPlateNum or args[1] == nil then
		TriggerEvent('radio')
		ShowNotification("~b~База данных LSPD: ~w~\nRunning ~o~" .. vehPlateNum .. "~w~." )
		Wait(2000)
		ShowNotification("~w~Рег. владелец: ~y~" .. regOwner .. "~w~\nРег. Год: ~y~" .. regYear .. "~w~\nFlags: ~y~" .. flags)
	else
		TriggerEvent('radio')
		ShowNotification("~o~База данных LSPD: ~w~\nRunning ~o~" .. args[1] .. "~w~." )
		Wait(2000)
		TriggerEvent('getInfo')
		ShowNotification("~w~Рег. владелец ~y~" .. rregOwner .. "~w~\nРег. Год: ~y~" .. rregYear .. "~w~\nFlags: ~y~" .. rflags)
	end
end)
...
AddEventHandler('pis:ticket', function()
	...
	ShowNotification("~o~Офицер:~w~ " .. "Я собираюсь выдать вам штраф в размере ~g~" .. price .. " ~w~за ~y~" .. reason)
	...
	if driverAttitude < 50 then
		DriverTicketQuotes = {"Alright..","I understand.","Да это нормально."}
	else
		DriverTicketQuotes = {"Oh come on!","You asshole.","All you cops do is fuck people over.","Go fight real crime."}
	end
		ShowNotification("~o~Водитель:~w~ " .. DriverTicketQuotes[math.random(#DriverTicketQuotes)])
end)
...
AddEventHandler('pis:askid', function()
	...
	if distanceToVeh <= 5 then
		if speech == "Normal" then
			...
			ShowNotification("~o~Офицер:~w~ " .. OfficerNormalQuotes[math.random(#OfficerNormalQuotes)])
		else
			local OfficerAggresiveQuotes = {"Show me your ID.", "Give me your ID.", "Give me your fucking license.", "Show me your info."}
			ShowNotification("~o~Офицер:~w~ " .. OfficerAggresiveQuotes[math.random(#OfficerAggresiveQuotes)])
		end
		Wait(2000)
		if lostIdChance > 95 then
			ShowNotification("~b~Водитель:~w~ Sorry officer, I don't have it on me...")
		else
			if driverAttitude < 50 then
			...
			ShowNotification("~b~Водитель:~w~ " .. driverResponseID[math.random(#driverResponseID)])
...
AddEventHandler('pis:exit', function()
	if stopped then
		Citizen.CreateThread(function()
			if speech == "Normal" then
				ShowNotification("~o~Офицер:~w~ Can you step out of the car for me, please?")
			else
				ShowNotification("~o~Офицер:~w~ Get the fuck out of the car.")
			end
			...
			if resistExitChance == 29 or resistExitChance == 25 or isPedGoingToFlee then
				...
				ShowNotification("~b~Водитель:~w~ ".. driverResponseResist[math.random(#driverResponseResist)])
				Citizen.Wait(5000)
				TriggerEvent("po:flee")
			else
				...
				ShowNotification("~b~Водитель:~w~ " .. driverResponseExit[math.random(#driverResponseExit)])
...
AddEventHandler('pis:mount', function()
	if stopped then
		ShowNotification("~o~Офицер:~w~ Get back in the car, please.")
		Citizen.CreateThread(function()
		RemovePedFromGroup(stoppedDriver)
...
AddEventHandler('pis:release', function()
	if stopped then
	if speech == "Normal" then
		ShowNotification("~o~Офицер:~w~ Alright, you're free to go.")
	else
		ShowNotification("~o~Офицер:~w~ Get out of here before i change my mind.")
	end
	if driverAttitude < 50 then
	...
	Wait(2000)
	ShowNotification("~b~Водитель:~w~ " .. driverResponseRelease[math.random(#driverResponseRelease)])
	TriggerEvent('po:release')
	end
end)
...
AddEventHandler('pis:warn', function()
	if stopped then
		...
		ShowNotification("~o~Офицер:~w~ " .. officerSays[math.random(#officerSays)])
		Wait(2000)
		ShowNotification("~b~Водитель:~w~ " .. driverResponseWarn[math.random(#driverResponseWarn)])
		Wait(2000)
		TriggerEvent('po:release')
	end
end)
...
AddEventHandler('pis:drunk:q', function()
	ShowNotification("~o~Офицер:~w~ " .. "Have you had anything to drink today?")
	if drunk == true then
	...
	ShowNotification("~b~Водитель:~w~ " .. driverResponseDrunk[math.random(#driverResponseDrunk)])
end)

RegisterNetEvent('pis:drug:q')
AddEventHandler('pis:drug:q', function()
	ShowNotification("~o~Офицер:~w~ " .. "Have you consumed any drugs recently?")
	...
	ShowNotification("~b~Водитель:~w~ " .. driverResponseDrug[math.random(#driverResponseDrug)])
end)

RegisterNetEvent('pis:illegal:q')
AddEventHandler('pis:illegal:q', function()
	ShowNotification("~o~Офицер:~w~ " .. "Is there anything illegal in the vehicle?")
	...
	ShowNotification("~b~Водитель:~w~ " .. driverResponseIllegal[math.random(#driverResponseIllegal)])
end)

RegisterNetEvent('pis:search:q')
AddEventHandler('pis:search:q', function()
	ShowNotification("~o~Офицер:~w~ " .. "Would you mind if i search your vehicle?")
	if search == true then
	...
	ShowNotification("~b~Водитель:~w~ " .. driverResponseSearch[math.random(#driverResponseSearch)])
end)
...
RegisterCommand("pit", function()
	if stoppedVeh ~= nil then
		...
		if fleeing == true then
			...
			ShowNotification("~b~Офицер:~w~ Requesting permission to pit the ~y~"..GetLabelText(GetDisplayNameFromVehicleModel(vehicleHash)).."~w~.")
