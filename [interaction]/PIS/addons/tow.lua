...
local towDriverQuoteOfTheDay = {
	"Привет, партнер! Я его отбуксирую.",
	"Ты вообще поднимаешь братан? что я делаю.",
	...
	"Вот и ты!",
	"Береги себя.",
	"Это будет хорошо если конфисковать!",
	"Пошел на хуй.",
	"Я понял!",
	"Спасибо за использование " .. companyName .. "!",
...
AddEventHandler( 'pis:spawnTow', function()
	...
	TriggerEvent('radio')
		...
		ShowNotification("Эвакуатор был отправлен к вам. Спасибо за использование ~y~" .. companyName .. "~w~\nETA: " .. eta)
		enroute = true
		while (enroute) do
			Citizen.Wait(300)
			...
			SetEntityInvincible(driver, true)
				if distanceToVeh <= 15 then
					...
					ShowNotification("~o~Водитель буксировки:~w~ " .. towDriverQuoteOfTheDay[math.random(#towDriverQuoteOfTheDay)])
	...
	ShowNotification("Транспортное средство не найдено!")
	end
end)

AddEventHandler( 'pis:cancelTow', function()
	if enroute == true then
		ShowNotification("Запрос на эвакуатор был отменен. Спасибо от ~y~" .. companyName)
