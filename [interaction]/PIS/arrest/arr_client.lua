				...
				if distanceToTarget <= 15 then
					...
					if IsPedInAnyVehicle(target) then -- if the target ped is in a vehicle...
						...
						if target == stoppedDriver then -- and if the ped is a driver during a pullover...
							ShowNotification("~o~Офицер:~w~ Быстро выйдти из транспорта")
							...
							if resistExitChance > 25 then
								...
								Citizen.Wait(500)
								local driverResponseResist = {"Ни за что!","Отвали!","Не сегодня!","Черт побери!","Хм .. Нет.","Да отвали ты от меня!","Свинья!","Нет.","Никогда!"}
...
AddEventHandler('pis:arr:pt', function()
    ...
    RequestModel(driver)
		...
		if DoesEntityExist(targetPed) then
			...
			ShowNotification('~o~Офицер: ~w~Отправка с просьбой о транспортировке заключенного на мой 20.')
			...
			ShowNotification('~o~Диспетчер: ~w~Транспортный фургон для заключенных был отправлен к вам. ~w~\nETA: ' .. eta)
			enroute = true
			while (enroute) do
			Citizen.Wait(300)
				...
				SetVehicleSiren(vehicle, true)
					...
					if GetEntitySpeed(vehicle) < 5 then
						...
						if isopen ~= 0 then
							...
							ShowNotification("~o~Офицер: ~w~Не роняй мыло!")
							...
							if IsPedInVehicle(targetPed, vehicle, false) then
								...
								ShowNotification("~o~Водитель:~w~ Я получил его отсюда.")
								enroute = false
