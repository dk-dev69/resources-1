...
local AnimAresztowany			= 'crook_p2_back_left'	-- Задержанный Анимация
...
Citizen.CreateThread(function()
	while true do
		Wait(0)
		--shift + e + правай кнопка мыши
		if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['E']) and IsControlPressed(0, 25) and not Aresztuje and GetGameTimer() - OstatnioAresztowany > 10 * 1000 and (PlayerData.job.name == PracaPolice or PlayerData.job.name == FBI) then	-- Mozesz tutaj zmienic przyciski
			Citizen.Wait(10)
				local closestPlayer, distance = ESX.Game.GetClosestPlayer()

				if distance ~= -1 and distance <= Config.ArrestDistance and not Aresztuje and not Aresztowany and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) and (PlayerData.job.name ~= PracaPolice or PlayerData.job.name ~= FBI) then
					Aresztuje = true
					OstatnioAresztowany = GetGameTimer()

					ESX.ShowNotification("Арест игрока ~b~ID:~r~ " .. GetPlayerServerId(closestPlayer) .. "")						-- Drukuje Notyfikacje
					...
					ESX.ShowNotification("Игрок ~b~ID: " .. GetPlayerServerId(closestPlayer) .. " ~r~арестован")					-- Drukuje Notyfikacje
